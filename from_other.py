...
def get_ETS_price(year: int) -> float:
    if year != 2021:
        ...
        ETS_ICE = get_ice_eua_prices()
        price = ETS_ICE.get(2020)
        return price
    
    elif year == 2021:
        ETS_ICE = get_ice_eua_prices()
        price = ETS_ICE.get(2021)
        return price
...
def _get_lignite_price(year: int = 2019, base_price: float = 6.3) -> float:
    ...
    fp = paths.DATA_DIR / "destatis/energiepreisentwicklung-xlsx-5619001_new.xlsx"
    xl = pd.ExcelFile(fp)
    df = xl.parse(
...
def _get_gas_price(year: int = 2019, country: str = "DE") -> float:
    ...
    Source: StatistischeBundesamt.2020 (https://www.destatis.de/DE/Themen/Wirtschaft/Preise/Publikationen/Energiepreise/energiepreisentwicklung-pdf-5619001.html)
    """
    #if country != "CZ" and country != "CH":# and country != "NO" and country != "PL":
        
        #fp = paths.DATA_DIR / "destatis/energiepreisentwicklung-xlsx-5619001.xls"
    fp = paths.DATA_DIR / "destatis/energiepreisentwicklung-xlsx-5619001_2022.xlsx"
    xl = pd.ExcelFile(fp)
    nblocks = 4
    #blocksize = 26
    blocksize = 31
    ...
    # elif country == "CZ":
    #     #           2018, 2019, 2020, 2021
    #     prices = [25.0241, 25.3, 21.45, 23.6]
    #     price = prices[year-2018]
    #     return price
    
    # elif country == "PL":
    #     #           2018, 2019, 2020, 2021
    #     prices = [25.2, 26.9, 21.45, 23.05]
    #     price = prices[year-2018]
    #     return price
    
    # elif country == "CH":
    #     #           2018, 2019, 2020, 2021
    #     prices = [65.7, 66.7, 64.2, 64.2] #only data until Q2 2020
    #     price = prices[year-2018]
    #     return price
    
    # elif country == "NO":
    #     #           2018, 2019, 2020, 2021
    #     prices = [13.1, 13.1, 13.1, 13.1]
    #     price = prices[year-2018]
    #     return price
    ...
    try:
        if country != "DK":
            ...
            return dict(
                    ...
                    lignite=_get_lignite_price(year=year),
                    nuclear=4.18,
                    biomass=24.96, #Assumed average of 2020 medium values for each three fuels from https://www.ea-energianalyse.dk/wp-content/uploads/2020/02/1280_analysis_of_biomass_prices.pdf
                )
...
def get_emissions_per_fuel_quaschning() -> Dict:
    ...
    #Updated using 2023 values
    return dict(oil=0.266, gas_cc=0.201, gas=0.201, coal=0.338, lignite=0.399)#, nuclear=0.0044*0.32)
