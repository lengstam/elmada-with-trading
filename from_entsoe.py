...
def prep_XEFs(year: int = 2019, freq: str = "60min", country: str = "DE") -> pd.DataFrame:
    ...
    #gas OC vs CC
    if 'gas' in shares_TF.columns:
        gas_share = eu_pwl.get_share_cc(country=country)
        shares_TF["gas_cc"] = shares_TF["gas"] * gas_share
        shares_TF["gas_oc"] = shares_TF["gas"] * (1-gas_share)
        shares_TF = shares_TF.drop("gas", axis=1)
    
    #life cycle emissions
    spec_emissions = load_el_national_specific_emissions_lc()

    if country == "SE1" or country == "SE2" or country == "SE3" or country == "SE4":
        ce_F=spec_emissions.iloc[:,25]
    else:
        try:
            ems_index = load_el_national_specific_emissions_lc().columns.get_loc(country)
            ce_F=spec_emissions.iloc[:,ems_index]
        except:
            #if country specific emission data is not available, use average
            ce_F = spec_emissions.sum(axis=1)/len(spec_emissions.columns)
            logger.warning(
                f"Country {country} not in Tranberg table for specific emission factors. "
                "Average is used."
            )
    
    # if country in spec_emissions or country == "SE1" or country == "SE2" or country == "SE3" or country == "SE4":
    ...
    ce_F = pd.concat((ce_F,ems_new), axis=0)
        
        
    #ce_F = spec_emissions[used_country]
    x = list(set(shares_TF.keys()) & set(ce_F.keys()))
...
#couldn't read the parquet files the above way so did as below
    if cache and fp.exists():
        df = hp.read(fp)
    else:
    ...
    if country == "SE1" or country == "SE2" or country == "SE3" or country == "SE4":
        if year == 2020:
            index = pd.date_range(start='2020-01-01 00:00',freq='H', periods=8784, tz='Europe/Berlin')
        elif year == 2018:
            index = pd.date_range(start='2018-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2019:
            index = pd.date_range(start='2019-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2021:
            index = pd.date_range(start='2021-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2022:
            index = pd.date_range(start='2022-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2030:
            index = pd.date_range(start='2030-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2050:
            index = pd.date_range(start='2050-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2051:
            index = pd.date_range(start='2051-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
        elif year == 2052:
            index = pd.date_range(start='2052-01-01 00:00',freq='H', periods=8760, tz='Europe/Berlin')
            #df = pd.read_parquet('2020_SE1_gen_entsoe.parquet', engine='pyarrow')
    ...
    return pd.DataFrame(
        {
            ...
            "hydro_res": df["hydropower_reservoir"],
            "hydro_ror": df["hydropower_run-of-river"],
            "lignite": df["coal"],
            "other_conv": df["coal"],
            "coal": df["coal"],
            "gas": df["gas"],
...
def load_el_national_specific_emissions_fix() -> pd.DataFrame:
    """Read specific emissions data from [Tranberg.2019] in gCO2eq/kWh.

    [Tranberg.2019]: https://doi.org/10.1016/j.esr.2019.100367
    """
    fp = paths.DATA_DIR / f"tranberg/specific_emission_factors_fix.csv"
    df = pd.read_csv(fp, header=0, index_col=0, skiprows=3).T
    return pd.DataFrame(
        {
            "wind_offshore": df["wind"],
...
def load_el_national_specific_emissions_lc() -> pd.DataFrame:
    """Read specific emissions data from [Tranberg.2019] in gCO2eq/kWh.

    [Tranberg.2019]: https://doi.org/10.1016/j.esr.2019.100367
    """
    fp = paths.DATA_DIR / f"tranberg/specific_emission_factors_lc.csv"
    df = pd.read_csv(fp, header=0, index_col=0, skiprows=3).T
    
    #Average carbon intensities using elmada range:
    lignite = np.zeros(28) + 1082.0
    coal = np.zeros(28) + 970.4
    gas_oc = np.zeros(28) + 649.3
    gas_cc = np.zeros(28) + 441.8
    oil = np.zeros(28) + 871.0
    return pd.DataFrame(
        {
            "wind_offshore": df["wind"],
            "wind_onshore": df["wind"],
            ...
            "hydro_res": df["hydropower_reservoir"],
            "hydro_ror": df["hydropower_run-of-river"],
            "lignite": lignite, #df["coal"],
            "other_conv": coal, #df["coal"],
            "coal": coal, #df["coal"],
            "gas": df["gas"],
            "oil": oil, #df["oil"],
            "solar": df["solar"],
