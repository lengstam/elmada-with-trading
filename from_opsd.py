...
def get_CEFs_from_merit_order(
    ...
    #if country == "SE4":
    #    total_load_T = pd.read_parquet('2020_SE4_gen_entsoe.parquet', engine='pyarrow').sum(axis=1)
    ...
    if country == "SE1" or country == "SE2" or country == "SE3" or country == "SE4":
        transm_eff = from_other.get_transmission_efficiency(country="SE")
    else:
    ...
    def get_data(resi_value: float, what: str = "marginal_emissions"):
        ...
        fuel_index = cols.index("fuel_draf")
        for row in range(len_mo):
            if mo_P_arr[row, 0] > resi_value:
                if what != "marginal_emissions":
                    return mo_P_arr[row, output_col_index]
                else:
                    mef_fuel = mo_P_arr[row, fuel_index]
                    return (mo_P_arr[row, output_col_index] + indir_ems[mef_fuel])[0]
        #LINUS EDIT, if no residual capacity return 0
        if len_mo == 0:
            return 0
        else:
            #was just the below statement before (one tab to the left)
    ...
    def get_emissions_for_xef(resi_value: float) -> float:
        ...
        emissions = 0.0
        for row in range(len_mo):
            fuel_type = mo_P_arr[row,3]
            #emissions += mo_P_arr[row, 1] * mo_P_arr[row, 2]
            emissions += (mo_P_arr[row, 1] + indir_ems.loc[0,fuel_type]) * mo_P_arr[row, 2]
            ...
            if mo_P_arr[row, 0] > resi_value:
                ...
                emissions -= (mo_P_arr[row, 1] + indir_ems.loc[0,fuel_type]) * (mo_P_arr[row, 0] - resi_value)
    ...
    #vector containing LC and upstream additions to direct emissions'
    #first term is upstream, second term infrastructure (Tranberg)
    #need to import infrastructure data!
    fix_ems = from_entsoe.load_el_national_specific_emissions_fix()
    if country == "SE1" or country == "SE2" or country == "SE3" or country == "SE4":
        fix_ems=fix_ems.iloc[:,25]
    else:
        try:
            fix_ems_index = from_entsoe.load_el_national_specific_emissions_fix().columns.get_loc(country)
            fix_ems=fix_ems.iloc[:,fix_ems_index]
        except:
            #if couontry specific emission data is not available, use average
            fix_ems = fix_ems.sum(axis=1)/len(fix_ems.columns)
            
    #From: https://www.sciencedirect.com/science/article/pii/S1361920916307933.
    #These values are for energy fuel input, so dividing with average efficiency to produce post-combustion values.
    #This is already included in biomass and nuclear because using life cycle emissions, so no need.
    if country != "DK" and country != "DE":
        indir_ems = pd.DataFrame({"coal": [((16*3.6/1000) / ((0.347+0.492)/2)) + fix_ems.loc["coal"]/1000],
                                 "lignite": [((1.7*3.6/1000) / ((0.322+0.449)/2)) + fix_ems.loc["lignite"]/1000],
                                 "gas_cc": [((12.8*3.6/1000) / ((0.516+0.612)/2)) + fix_ems.loc["gas"]/1000],
                                 "gas": [((12.8*3.6/1000) / ((0.328+0.450)/2)) + fix_ems.loc["gas"]/1000],
                                 "oil": [((10.7*3.6/1000) / ((0.314+0.395)/2)) + fix_ems.loc["oil"]/1000]})
    elif country == "DK":
        indir_ems = pd.DataFrame({"coal": [((16*3.6/1000) / ((0.347+0.492)/2)) + fix_ems.loc["coal"]/1000],
                                 "lignite": [((1.7*3.6/1000) / ((0.322+0.449)/2)) + fix_ems.loc["lignite"]/1000],
                                 "gas_cc": [((12.8*3.6/1000) / ((0.516+0.612)/2)) + fix_ems.loc["gas"]/1000],
                                 "gas": [((12.8*3.6/1000) / ((0.328+0.450)/2)) + fix_ems.loc["gas"]/1000],
                                 "oil": [((10.7*3.6/1000) / ((0.314+0.395)/2)) + fix_ems.loc["oil"]/1000],
                                 "biomass": [fix_ems.loc["biomass"]/1000]})
    elif country == "DE":
        indir_ems = pd.DataFrame({"coal": [((16*3.6/1000) / ((0.347+0.492)/2)) + fix_ems.loc["coal"]/1000],
                                 "lignite": [((1.7*3.6/1000) / ((0.322+0.449)/2)) + fix_ems.loc["lignite"]/1000],
                                 "gas_cc": [((12.8*3.6/1000) / ((0.516+0.612)/2)) + fix_ems.loc["gas"]/1000],
    ...
    # upstream_ems = pd.DataFrame({"coal": [16*3.6/1000],
    #                          "lignite": [1.7*3.6/1000],
    #                          "gas_cc": [12.8*3.6/1000],
    #                          "gas": [12.8*3.6/1000],
    #                          "oil": [10.7*3.6/1000]})    
    ...
    #Non-residual emissions
    se_res_ems_tot = []
    #these are LC emissions
    ems = from_entsoe.load_el_national_specific_emissions_lc()
    if country == "SE1" or country == "SE2" or country == "SE3" or country == "SE4":
        ems_se=ems.iloc[:,25]
    else:
        try:
            ems_index = from_entsoe.load_el_national_specific_emissions_lc().columns.get_loc(country)
            ems_se=ems.iloc[:,ems_index]
        except:
            #if country specific emission data is not available, use average
            ems_se = ems.sum(axis=1)/len(ems.columns)
    #LC emissions
    ems_new = pd.Series([1089.2, 329.5, 470.8, 0, 46.9, 895.8, 367], index=["peat", "chp", "se_gas", "other_mixed", "grot", "se_coal", "waste"])
    #see supplementary information for source
    ems_new = pd.Series(ems_new)
    ems_se = pd.concat((ems_se,ems_new), axis=0)
    tot_load = from_entsoe.load_el_national_generation(
        year=year, freq=freq, country=country)
    res_types = list(tot_load.columns.values)
        
    for h in range(len(tot_load)):
        se_res_emissions = 0
            
        for i in range(len(res_types)):
            
            #exclude DE nuclear and DK biomass as they are part of merit order emissions
            if country != "DK" and country != "DE":
                if bool(res_types[i] in mp.RES) is True:
                    se_res_emissions += (ems_se.loc[res_types[i]]*tot_load.loc[h,res_types[i]])
                    
            elif country == "DK":
                if bool(res_types[i] in mp.RES) is True and res_types[i] != "biomass":
                    se_res_emissions += (ems_se.loc[res_types[i]]*tot_load.loc[h,res_types[i]])
            
            elif country == "DE":
                if bool(res_types[i] in mp.RES) is True and res_types[i] != "nuclear":
                    se_res_emissions += (ems_se.loc[res_types[i]]*tot_load.loc[h,res_types[i]])
            
                    
        se_res_ems_tot.append(se_res_emissions)

    se_res_ems_tot = pd.Series(se_res_ems_tot) / transm_eff
    df["XEFs"] = ((df["XEFs"] * tot_load.sum(axis=1)) + se_res_ems_tot) / tot_load.sum(axis=1)
